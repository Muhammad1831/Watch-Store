abstract class AppString {
  AppString._();

  // ارسال کد فعال سازی
  static const String enterYourNumber = 'شماره خود را وارد کنید';
  static const String numberHintText = 'مثال :  09121114466';
  static const String sendActivityCode = 'ارسال کد فعال سازی';
  static const String userPhoneNumber = '09121114466';
  static const String numberIsNotEnterCorrect = 'شماره را درست وارد نکردید';

  // تایید کد فعال سازیی
  static const String sendedActivityCodeFor =
      'کد فعال سازی برای $userPhoneNumber ارسال شد';
  static const String numberIsWrongEditing = 'شماره اشتباه است/ ویرایش شماره';
  static const String enterActivityCode = 'کد فعال سازی را وارد کنید';
  static const String hintActivityCode = '-  -  -  -  -  -';
  static const String next = 'ادامه';
  static const String codeIsNotEnterCorrect = 'کد را درست وارد نکردید';

  // ثبت نام کاربر عادی
  static const String register = 'ثبت نام ';
  static const String chooseProfileImage = 'انتخاب تصویر پروفایل';
  static const String firstNamelastName = 'نام و نام خانوادگی';
  static const String firstNamelastNameHint =
      'نام و نام خانوادگی خود را با حروف فارسی وارد کنید';
  static const String homePhone = 'تلفن ثابت';
  static const String homePhoneHint =
      'شماره تلفن ثابت را با پیش شماره وارد بفرمایید';
  static const String address = 'آدرس';
  static const String addressHint = 'آدرس پستی خود را وارد کنید';
  static const String postalCode = 'کد پستی';
  static const String postalCodeHint = 'کد پستی  10 رقمی خود را وارد کنید';
  static const String location = 'موقعیت مکانی';
  static const String locationHint = 'برای انتخاب موقعیت مکانی ضربه بزنید';

  // home
  static const String home = 'خانه';
  static const String bascket = 'سبد خرید';
  static const String profile = 'پروفایل';
  static const String searchProduct = 'جستجوی محصولات';
  static const String classic = 'کلاسیک';
  static const String smart = 'هوشمند';
  static const String digital = 'دیجیتال';
  static const String desktop = 'رو‌میزی';

  // product list
  static const String ordering = 'مرتب سازی';
  static const String newest = 'جدیدترین‌ها';
  static const String cheapest = 'ارزان‌ترین‌ها';
  static const String mostExpensive = 'گران‌ترین‌ها';
  static const String mostViewed = 'پربازدید‌ترین‌ها';

  // details product
  static const String features = 'خصوصیات';
  static const String comments = 'نظرات';
  static const String reviews = 'نقد و بررسی';
  static const String addToBasket = 'افزودن به سبد خرید';

  // basket
  static const String sendToAddress = 'ارسال به آدرس';
  static const String price = 'قیمت';
  static const String withOff = 'با تخفیف';
  static const String continueBuyProcess = 'ادامه فرآیند خرید';
  static const String total = 'مجموع';
  static const String totalWithDiscount = 'مجموع با تخفیف';
  static const String tryAgain = 'تلاش مجدد';

  // profile
  static const String userProfile = 'پروفایل کاربر';
  static const String termsConditions = 'قوانین و مقررات';
  static const String delivered = 'تحویل شده';
  static const String cancled = 'لغو شده';
  static const String processing = 'در حال پردازش';
  static const String activeAddress = 'آدرس فعال';

  // search
  static const String dontFindResults = 'نتیجه‌ای یافت نشد';
}
